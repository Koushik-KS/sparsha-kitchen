...
export default function TrackOrderClient() {
  ...
  const fetchOrder = useCallback(
    async (event?: FormEvent<HTMLFormElement>) => {
      ...
      if (!trimmedPhone) {
        setError("Please enter the phone number used for the order.");
        return;
      }
      ...
      try {
        ...
        if (!response.ok || !data.success) {
          throw new Error(
            data.message || "Unable to find the order."
          );
        }
  ...
  const handleConfirmOrder = async () => {
    ...
    try {
      ...
      if (!response.ok || !data.success) {
        throw new Error(
          data.message || "Unable to confirm the order."
        );
      }
      ...
      if (returnedOrder) {
        setOrder((previousOrder) => {
          if (!previousOrder) return returnedOrder;

          return {
            ...previousOrder,
            ...returnedOrder,
            customer:
              returnedOrder.customer || previousOrder.customer,
            items: returnedOrder.items || previousOrder.items,
            deliveryPerson:
            ...
            confirmationToken:
              returnedOrder.confirmationToken ?? null,
          };
        });
      } else {
        await fetchOrder();
  ...
  const handleChangeRequest = async (
    ...
    if (!order?.trackingToken) {
      setError("Tracking token is not available for this order.");
      return;
    }

    if (!changeMessage.trim()) {
      setError("Please enter what you would like to change.");
      return;
    }
    ...
    try {
      ...
      if (!response.ok || !data.success) {
        throw new Error(
          data.message || "Unable to submit the change request."
        );
      }
    ...
    <main className="min-h-screen bg-orange-50 text-zinc-900">
      ...
      <section className="mx-auto max-w-5xl px-6 py-14 lg:px-8">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <p className="mt-5 text-lg leading-8 text-zinc-600">
            Enter your Order ID and phone number to view the latest status
            of your order.
          </p>
        </div>

        <div className="mx-auto mt-10 max-w-2xl rounded-3xl border border-orange-100 bg-white p-7 shadow-sm sm:p-9">
          <form onSubmit={fetchOrder} className="space-y-5">
            <div>
              <label
                htmlFor="orderId"
          ...
          <div className="mt-10 space-y-7">
              ...
              <div className="rounded-3xl border border-orange-100 bg-white p-7 shadow-sm">
                ...
                <div className="mt-5 space-y-4 text-sm">
                  ...
                  <div>
                    ...
                    <p className="mt-1 font-semibold">
                      {order.requestedDeliveryTime || "Not specified"}
                    </p>
                  </div>
            ...
            <div className="rounded-3xl border border-orange-100 bg-white p-7 shadow-sm sm:p-9">
              ...
              <div className="mt-5 border-t border-zinc-200 pt-5">
                <div className="flex justify-between text-sm text-zinc-600">
                  ...
                  <span>
                    ₹{Number(order.foodTotal || 0).toFixed(2)}
                  </span>
                </div>
              ...
              <div className="rounded-3xl border border-orange-100 bg-white p-7 shadow-sm">
                ...
                <div className="mt-5 grid gap-4 sm:grid-cols-2">
                  <div>
                    ...
                    <p className="mt-1 font-semibold">
                      {order.deliveryPerson.name || "Not available"}
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="mt-1 font-semibold">
                      {order.deliveryPerson.phone || "Not available"}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-3xl border border-green-200 bg-green-50 p-7">
                ...
                {order.customerConfirmedAt && (
                  <p className="mt-1 text-sm text-green-700">
                    Confirmed {formatDateTime(order.customerConfirmedAt)}
                  </p>
                )}
              </div>
            )}
              ...
              <div className="rounded-3xl border border-orange-100 bg-white p-7 shadow-sm">
                ...
                {!order.changeRequested && (
                  ...
                  >
                    ...
                    >
                      {requestingChange ? "Sending..." : "Request Change"}
                    </button>
                  </form>
                )}
              </div>
            ...
            {canRequestCancellation && (
              <div className="rounded-3xl border border-red-100 bg-white p-7 shadow-sm">
                <h2 className="text-xl font-bold">Request Cancellation</h2>

                <p className="mt-2 text-sm leading-6 text-zinc-600">
                  If you no longer need this order, you can send a cancellation
                  request.
                </p>
      ...
      <footer className="border-t border-orange-100 bg-white">
        <div className="mx-auto flex max-w-7xl flex-col gap-3 px-6 py-8 text-sm sm:flex-row sm:items-center sm:justify-between lg:px-8">
          <div>
            <div className="font-bold text-orange-600">Sparsha Kitchen</div>
